...
namespace ft_ext {
template<typename T>
class GptService: public ModelService {
private:
    int                     OnInit() override;
    ModelPtr                NewModel() override;
    ModelServiceInstancePtr NewModelInstance(int                                     device_id,
                                             const Model*                            model,
                                             const ModelServiceConfig&               config,
                                             ft::NcclParam                           tensor_para,
                                             ft::NcclParam                           pipeline_para,
                                             std::shared_ptr<ft::AbstractCustomComm> custom_comm) override;
};
}  // namespace ft_ext