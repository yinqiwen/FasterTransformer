...
int32_t BertTokenizerVocab::FindTokenId(const ustring& token)
{
    ...
    if (it == vocab_.end()) {
        return -1;
        // ORTX_CXX_API_THROW("[BertTokenizerVocab]: can not find tokens: " + std::string(token),
...
WordpieceTokenizer::WordpieceTokenizer(std::shared_ptr<BertTokenizerVocab> vocab,
...
std::vector<int> WordpieceTokenizer::Encode(const std::vector<ustring>& tokens)
{
    std::vector<int> ids;
    for (const auto& token : tokens) {
        int32_t token_id = -1;
...
void TruncateStrategy::Truncate(std::vector<int64_t>& ids1, std::vector<int64_t>& ids2, int32_t max_len)
{
    ...
    switch (strategy_) {
        ...
        case TruncateStrategyType::LONGEST_FROM_BACK:
            ...
            if ((ids1_keep_len > static_cast<size_t>(half_max_len))
                && (ids2_keep_len > static_cast<size_t>(half_max_len))) {
...
BertTokenizer::BertTokenizer(const std::string& vocab,
                             bool               do_lower_case,
                             bool               do_basic_tokenize,
                             const ustring&     unk_token,
                             const ustring&     sep_token,
                             const ustring&     pad_token,
                             const ustring&     cls_token,
                             const ustring&     mask_token,
    ...
    truncate_(std::make_unique<TruncateStrategy>(truncation_strategy))
{
    ...
    if (do_basic_tokenize) {
        basic_tokenizer_ =
            std::make_unique<BasicTokenizer>(do_lower_case, tokenize_chinese_chars, strip_accents, true, true);
    }
    wordpiece_tokenizer_ = std::make_unique<WordpieceTokenizer>(vocab_, unk_token, suffix_indicator);
...
std::vector<int> BertTokenizer::Encode(const std::vector<ustring>& tokens)
{
...
std::vector<int> BertTokenizer::AddSpecialToken(const std::vector<int>& ids)
{
...
std::vector<int> BertTokenizer::AddSpecialToken(const std::vector<int>& ids1, const std::vector<int>& ids2)
{
    std::vector<int> result;
...
std::vector<int> BertTokenizer::GenerateTypeId(const std::vector<int>& ids)
{
    return std::vector<int>(ids.size() + 2, 0);
}

std::vector<int> BertTokenizer::GenerateTypeId(const std::vector<int>& ids1, const std::vector<int>& ids2)
{
    std::vector<int> result;
    result.reserve(ids1.size() + ids2.size() + 3);
...
void BertTokenizer::GetWordIdList(const std::vector<std::vector<std::string>>& word_dict,
                                  std::vector<std::vector<std::vector<int>>>&  word_ids)
{
    std::vector<std::vector<int>> flat_ids;
    std::vector<std::vector<int>> offsets;
    for (const auto& word_dict_item : word_dict) {
        std::vector<int> item_flat_ids;
        std::vector<int> item_offsets;
        for (const auto& word : word_dict_item) {
            auto ids = Encode(Tokenize(ustring(word)));
            item_flat_ids.insert(item_flat_ids.end(), ids.begin(), ids.end());
            item_offsets.emplace_back(ids.size());
        }
        flat_ids.emplace_back(item_flat_ids);
        std::vector<int> cum_sum(item_offsets.size());
        std::partial_sum(item_offsets.begin(), item_offsets.end(), cum_sum.begin());
        offsets.emplace_back(cum_sum);
    }
    size_t max_pad_len = 1;
    for (auto& ids : flat_ids) {
        if (ids.size() > max_pad_len) {
            max_pad_len = ids.size();
        }
    }
    for (size_t i = 0; i < flat_ids.size(); i++) {
        while (flat_ids[i].size() < max_pad_len) {
            flat_ids[i].emplace_back(0);
        }
        while (offsets[i].size() < max_pad_len) {
            offsets[i].emplace_back(-1);
        }
    }

    for (size_t i = 0; i < word_dict.size(); i++) {
        std::vector<std::vector<int>> item;
        item.emplace_back(flat_ids[i]);
        item.emplace_back(offsets[i]);
        word_ids.emplace_back(item);
    }
}
